#include "../../entity.h"
#include "menuselector.h"
#include "../../timesystem.h"
#include "../../scene.h"
#include "../../scenecontroller.h"
#include <stdio.h>
#include "../../animation.h"

#include "startbutton.h"
#include "optionbutton.h"
#include "exitbutton.h"

#include <SDL3/SDL.h>

void menuselector_start();
void menuselector_poll(SDL_Event* event);
void menuselector_loop();
void menuselector_render(SDL_Renderer* renderer);
void menuselector_destroy();

Entity* menuselector_buttons[3];
int menuselector_selectedIndex = 0;

void menuselector_init(){
    menuselector = ui_create_image(
            "res/ui/jaramedflag.png",
            &menuselector_start,
            &menuselector_poll,
            &menuselector_loop,
            &menuselector_render,
            &menuselector_destroy
        );
}

void menuselector_start(){
    // printf("this will run once");
    menuselector->anchorX = -5;
    menuselector->anchorY = -5;
    menuselector->img->imgSizeX = 100;
    menuselector->img->imgSizeX = 100;
    menuselector->w = 10;
    menuselector_buttons[0] = startbutton;
    menuselector_buttons[1] = optionbutton;
    menuselector_buttons[2] = exitbutton;
}
void menuselector_poll(SDL_Event* event){
    if(event->type == SDL_EVENT_KEY_DOWN){
        if(event->key.scancode == SDL_SCANCODE_W){
            if(menuselector_selectedIndex - 1 < 0){
                menuselector_selectedIndex = 3;
            }
            menuselector_selectedIndex--;
        }
        else if(event->key.scancode == SDL_SCANCODE_S){
            if(menuselector_selectedIndex + 1 > 2){
                menuselector_selectedIndex = -1;
            }
            menuselector_selectedIndex++;
        }
        else if(event->key.scancode == SDL_SCANCODE_Q){
            switch(menuselector_selectedIndex){
                case 0:
                    startbutton_interact();
                    break;
                case 1:
                    optionbutton_interact();
                    break;
                case 2:
                    exitbutton_interact();
                    break;
            }
        }
    }
}

void menuselector_loop(){
    menuselector->x = menuselector_buttons[menuselector_selectedIndex]->x + menuselector->w / 2;
    menuselector->y = menuselector_buttons[menuselector_selectedIndex]->y + menuselector->h;
}
void menuselector_render(SDL_Renderer* renderer){
    render_entity(menuselector, renderer, NULL);
}
void menuselector_destroy(){
    destroy_entity(menuselector);
}
